...
if(!empty($destino)){
	...
	// Lets work with it
if (!wasRecentlyRegistredLead($email)){
	//BEGIN EMAIL
...
function wasRecentlyRegistredLead($lead){
	$conn = getConnection();
	$recentlyRegistredLead = 0;

	$query = sprintf("SELECT * FROM lastleads WHERE lead_email = '%s' limit 1", $lead);

	$msg2log = sprintf(  "Busca cadastro de lead: %s " , $query );
	logMsg( $msg2log );
	if ($result = $conn->query($query)) {
		while ($row = $result->fetch_row()) {
			$recentlyRegistredLead = 1;
		}
		/* free result set */
		$result->close();
	}
	else{
		$msg2log = sprintf( "[wasRecentlyRegistredLead]Não há resultado para busca. Lead: %s " , $lead );
		logMsg( $msg2log );
	}
	$recentlyRegistredLead = $recentlyRegistredLead!=""?1:0;
	$msg2log = sprintf( "[wasRecentlyRegistredLead] %s " , $recentlyRegistredLead );
	logMsg( $msg2log );

	return $recentlyRegistredLead;
}
