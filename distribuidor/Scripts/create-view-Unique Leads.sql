...
/*!50001 VIEW `uniqueleads` AS select distinct `a`.`lead_email` AS `email`,`a`.`lead_name` AS `nome`,`a`.`lead_phone` AS `celular`,`a`.`consulting_id` AS `codigoConsultor`,`c`.`consulting_name` AS `nomeConsultor`,`a`.`interest` AS `interest`,`a`.`ip` AS `ip`,`a`.`from_url` AS `url`,`a`.`origins` AS `codigoOrigem`,`a`.`product` AS `codigoProduto`,`a`.`id_leadlovers` AS `idLeadlovers`,`a`.`pid_leadlovers` AS `pidLeadlovers`,`a`.`observation` AS `observacao`,`a`.`thanks_url` AS `paginaObrigado`,`a`.`first_time` AS `primeiraAplicacao`,`a`.`whatsapp` AS `whatsapp`,extract(day from `a`.`lead_register_date`) AS `dia`,extract(hour from `a`.`lead_register_date`) AS `hora`,extract(minute from `a`.`lead_register_date`) AS `minutos`,date_format(`a`.`lead_register_date`,'%Y%m%d') AS `data` from (`access` `a` left join `consulting01` `c` on((`a`.`consulting_id` = `c`.`ID`))) */;
...
create view uniqueLeads 
 ...
 `a`.`ip` AS `ip`,`a`.`from_url` AS `url`,`a`.`origins` AS `codigoOrigem`,
 `a`.`product` AS `codigoProduto`,`a`.`id_leadlovers` AS `idLeadlovers`,
